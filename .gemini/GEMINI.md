...
9. **Sandbox environment** — Authentication is handled automatically. `login`, `logout`, `init`, `setup` commands are NOT available. Read `.gemini/skills/schema0-cli/SKILL.md` before running any `schema0` command.

## Architecture

```
packages/db/      — Database schemas (Drizzle ORM)        → see packages/db/GEMINI.md
packages/api/     — ORPC API routers                      → see packages/api/GEMINI.md
packages/auth/    — Server-side auth (uses @schema0/auth-web)
packages/test/    — Integrated tests (PGlite + UI)        → see packages/test/GEMINI.md
...
Read `packages/test/GEMINI.md` in full before writing any test code.
