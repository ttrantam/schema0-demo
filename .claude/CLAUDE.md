...
9. **Sandbox environment** — Authentication is handled automatically. `login`, `logout`, `init`, `setup` commands are NOT available. Read `.claude/skills/schema0-cli/SKILL.md` before running any `schema0` command.

## Architecture

```
packages/db/      — Database schemas (Drizzle ORM)        → see packages/db/CLAUDE.md
packages/api/     — ORPC API routers                      → see packages/api/CLAUDE.md
packages/auth/    — Server-side auth (uses @schema0/auth-web)
packages/test/    — Integrated tests (PGlite + UI)        → see packages/test/CLAUDE.md
...
Read `packages/test/CLAUDE.md` in full before writing any test code.
