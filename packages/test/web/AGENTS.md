...
| ------------------------------ | -------------------------------------------- | ------------------------------------------------------------------------- |
| ECONNREFUSED in router         | Router uses `fetchCustomResources`           | Rewrite router to use `createDb()` — see `packages/api/AGENTS.md`         |
| Schema validation rejects data | Missing fields or nullable/optional mismatch | Fix the schema overrides — see `packages/db/AGENTS.md`                    |
| `onInsert` never called        | Collection schema validation gate            | Add system fields before `collection.insert()` — see `apps/web/AGENTS.md` |
